...
const AdminDashboard = () => {
    ...
    const handleDeleteAllQuestions = async () => {
        if (!stats?.questions.total || stats.questions.total === 0) {
            alert('No questions found in database to delete.');
            return;
        }

        const confirmText = prompt(
            `⚠️ DANGER ZONE ⚠️\n\nThis will permanently delete ALL ${stats?.questions.total || 0} questions from the database.\n\nType "DELETE ALL" to confirm:`
        );
                    ...
                    <div className="mt-6 p-4 border-2 border-red-200 rounded-2xl bg-red-50">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <p className="font-semibold text-red-800">Delete All Questions</p>
                                    <p className="text-sm text-red-600">Permanently remove all {stats?.questions.total || 0} questions from the database</p>
                                </div>
                            </div>
                            <button
                                onClick={handleDeleteAllQuestions}
                                disabled={deleting}
                                className={`px-6 py-3 rounded-xl font-semibold text-white transition-all duration-200 ${deleting
