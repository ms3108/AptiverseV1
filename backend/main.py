...
def submit_answer(
    ...
    # 🎯 NEW: Recalculate question difficulty every 10 attempts
    total_attempts = db.query(models.QuestionAttempt).filter(
        models.QuestionAttempt.question_id == question.id
    ).count()
    
    if total_attempts % 10 == 0:
        try:
            difficulty_result = difficulty_algorithm.update_question_difficulty(
                question_id=question.id,
                db=db,
                force=False
            )
            
            if difficulty_result.get('success') and difficulty_result.get('difficulty_changed'):
                print(f"✨ Question {question.id} difficulty updated: "
                      f"{difficulty_result['old_difficulty']} → {difficulty_result['new_difficulty']} "
                      f"(confidence: {difficulty_result['confidence']:.2f})")
        except Exception as e:
            print(f"⚠️ Error updating difficulty for question {question.id}: {e}")
...
# ========================================================================
# 🎯 DIFFICULTY ALGORITHM ENDPOINTS
# ========================================================================

@app.get("/admin/difficulty-analysis")
def get_difficulty_analysis(
    current_user: models.User = Depends(auth.get_current_user),
    db: Session = Depends(get_db)
):
    """
    Admin endpoint: Get comprehensive difficulty distribution analysis
    
    Returns:
        - Total questions by difficulty
        - Confidence distribution
        - Questions needing review (low confidence)
        - Recently reclassified questions
    """
    import difficulty_algorithm
    ...
    for q in questions:
        # Count by difficulty
        if q.difficulty in analysis['by_difficulty']:
            analysis['by_difficulty'][q.difficulty] += 1
        
        # Confidence distribution
        if q.difficulty_confidence is not None:
            conf_bucket = f"{int(q.difficulty_confidence * 10) * 10}-{int(q.difficulty_confidence * 10) * 10 + 10}%"
            analysis['confidence_distribution'][conf_bucket] = analysis['confidence_distribution'].get(conf_bucket, 0) + 1
            
            # Low confidence flagging
            if q.difficulty_confidence < 0.5:
                analysis['needs_review'].append({
                    'id': q.id,
                    'title': q.title,
                    'difficulty': q.difficulty,
                    'difficulty_score': q.difficulty_score,
                    'confidence': q.difficulty_confidence,
                    'tier_stats': q.tier_stats,
                    'total_attempts': q.total_attempts
                })
            elif q.difficulty_confidence >= 0.7:
...
@app.get("/admin/recalculate-difficulties")
def recalculate_all_question_difficulties(
    current_user: models.User = Depends(auth.get_current_user),
    db: Session = Depends(get_db)
):
    """
    Admin endpoint: Recalculate all question difficulties
    
    This should be run periodically (e.g., weekly) to keep difficulties current
    """
    import difficulty_algorithm
    ...
    stats = difficulty_algorithm.recalculate_all_difficulties(db, min_confidence=0.3)
    
    return {
        'success': True,
        'message': f"Recalculated {stats['updated']} questions, {stats['reclassified']} changed difficulty",
        'stats': stats
    }


@app.get("/question/{question_id}/difficulty-details")
def get_question_difficulty_details(
    question_id: int,
    current_user: models.User = Depends(auth.get_current_user),
    db: Session = Depends(get_db)
):
    """
    Get detailed difficulty breakdown for a specific question
    
    Returns:
        - Current difficulty and score
        - Confidence level
        - Per-tier statistics
        - Difficulty history
        - Personalized difficulty for current user
    """
    import difficulty_algorithm
    ...
    result = difficulty_algorithm.calculate_skill_weighted_difficulty(question_id, db)
    
    # Get personalized difficulty
    personalized = difficulty_algorithm.get_personalized_difficulty(question_id, current_user.id, db)
    ...
    return {
        ...
        'confidence': question.difficulty_confidence,
        'xp_reward': question.xp_reward,
        'total_attempts': question.total_attempts,
        'tier_breakdown': result['breakdown'],
        'personalized_difficulty': personalized,
        'history': question.difficulty_history or []
    }


@app.post("/admin/force-update-difficulty/{question_id}")
def force_update_question_difficulty(
    question_id: int,
    current_user: models.User = Depends(auth.get_current_user),
    db: Session = Depends(get_db)
):
    """
    Admin endpoint: Force update a specific question's difficulty
    (bypasses confidence threshold)
    """
    import difficulty_algorithm
    ...
    result = difficulty_algorithm.update_question_difficulty(
        question_id=question_id,
        db=db,
        force=True
    )
